...
export default function NilaiView({ currentUser, apiRequest, showToast, onDeleteTrigger, modalOpenTrigger, setModalOpenTrigger }) {
  ...
  const getFilteredGrades = () => {
    ...
    if (currentUser.role === 'dosen' && matchingDosen) {
      ...
      result = result.filter((n) => filteredCourses.some((c) => c.id === n.mata_kuliah_id));
  ...
  return (
    <section id="view-nilai" className="view-section active">
      <div className="table-header-bar">
        <div className="search-box" style={{ display: 'flex', gap: '8px', flexWrap: 'wrap', width: 'auto', flexGrow: 1 }}>
          {/* Hide student filter for students */}
          {!isStudent && (
            <select
              id="filter-nilai-mahasiswa"
              className="form-control"
              style={{ maxWidth: '250px', backgroundColor: 'var(--bg-secondary)', borderColor: 'var(--border-color)' }}
              value={filterMhs}
          ...
          >
            <option value="">
              {isDosen ? '-- Filter MK yang Diajar (Semua) --' : '-- Filter Mata Kuliah (Semua) --'}
            </option>
          ...
          <button id="btn-reset-filter-nilai" className="btn btn-secondary" onClick={handleResetFilter}>
        ...
        {!isStudent && (
          <button id="btn-add-nilai" className="btn btn-primary" onClick={handleOpenAddModal}>
          ...
          <tbody id="table-body-nilai">
            {loading ? (
              ...
              displayedGrades.map((n) => {
                const { badgeClass, predikat } = getBadgeAndPredicate(n.nilai_huruf);
                return (
                  <tr key={n.id}>
                    <td style={{ fontWeight: 600 }}>{n.nama_mahasiswa || <span className="text-muted">-</span>}</td>
                    <td>{n.nama_mata_kuliah || <span className="text-muted">-</span>}</td>
                    <td style={{ fontWeight: 700, fontSize: '15px', color: 'var(--primary)' }}>{n.nilai_angka}</td>
                    <td style={{ fontWeight: 800, fontSize: '16px', textAlign: 'center' }}>
                      <span
                        style={{
        ...
        <div className="modal-backdrop active" onClick={(e) => e.target.className.includes('modal-backdrop') && setModalOpen(false)}>
          <div className="modal-card">
            <div className="modal-header">
              <h3 id="modal-nilai-title">{editId ? 'Edit Nilai Mahasiswa' : 'Input Nilai Mahasiswa'}</h3>
