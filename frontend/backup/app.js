  ...
  try {
    ...
    if (!response.ok) {
      ...
      const errorMsg =
        data?.detail ||
        data?.message ||
        `Error ${response.status}: Terjadi kesalahan server.`;
...
async function initAuth() {
  if (token) {
    try {
      ...
      showToast(
        "Selamat Datang",
        `Halo, ${currentUser.nama || currentUser.username}!`,
        "success",
      );
      loadView("dashboard");
    } catch (err) {
...
async function checkDatabaseState() {
  try {
    const stats = await fetch(`${API_BASE}/api/dashboard`).then((r) =>
      r.json(),
    );
...
async function handleLogin(e) {
  ...
  // const role = document.getElementById('login-role').value;  // ← HAPUS BARIS INI!
  ...
  try {
    ...
    showToast(
      "Login Berhasil",
      `Selamat datang kembali, ${currentUser.nama || currentUser.username}.`,
      "success",
    );
...
async function loadDashboardData() {
  try {
    ...
    // Update labels
    document.getElementById("stat-total-mahasiswa").textContent =
      stats.total_mahasiswa;
    document.getElementById("stat-total-dosen").textContent = stats.total_dosen;
    document.getElementById("stat-total-matakuliah").textContent =
...
async function loadMahasiswaList(search = "") {
  ...
  tbody.innerHTML =
    '<tr><td colspan="6" style="text-align: center;"><i class="fa-solid fa-spinner fa-spin"></i> Memuat data...</td></tr>';

  try {
    const query = search
      ? `/api/mahasiswa?search=${encodeURIComponent(search)}`
      : "/api/mahasiswa";
    const students = await apiRequest(query);

    if (students.length === 0) {
      tbody.innerHTML =
        '<tr><td colspan="6" style="text-align: center; color: var(--text-muted);">Tidak ada data mahasiswa ditemukan.</td></tr>';
      return;
    }
...
async function editMahasiswa(id) {
  try {
    ...
    document.getElementById("modal-mahasiswa-title").textContent =
      "Edit Data Mahasiswa";
...
async function loadDosenList(search = "") {
  ...
  try {
    const query = search
      ? `/api/dosen?search=${encodeURIComponent(search)}`
      : "/api/dosen";
    const lecturers = await apiRequest(query);

    if (lecturers.length === 0) {
      tbody.innerHTML =
        '<tr><td colspan="6" style="text-align: center; color: var(--text-muted);">Tidak ada data dosen ditemukan.</td></tr>';
      return;
    }
...
async function saveDosen(e) {
  ...
  const keahlian =
    document.getElementById("dosen-keahlian").value.trim() || null;
...
async function editDosen(id) {
  try {
    ...
    document.getElementById("modal-dosen-title").textContent =
      "Edit Data Dosen";
...
async function loadMataKuliahList(search = "") {
  ...
  tbody.innerHTML =
    '<tr><td colspan="5" style="text-align: center;"><i class="fa-solid fa-spinner fa-spin"></i> Memuat data...</td></tr>';

  try {
    const query = search
      ? `/api/matakuliah?search=${encodeURIComponent(search)}`
      : "/api/matakuliah";
    const courses = await apiRequest(query);

    if (courses.length === 0) {
      tbody.innerHTML =
        '<tr><td colspan="5" style="text-align: center; color: var(--text-muted);">Tidak ada mata kuliah ditemukan.</td></tr>';
      return;
    }
...
async function editMataKuliah(id) {
  try {
    ...
    document.getElementById("modal-matakuliah-title").textContent =
      "Edit Mata Kuliah";
...
async function loadNilaiList() {
  ...
  try {
    ...
    if (grades.length === 0) {
      tbody.innerHTML =
        '<tr><td colspan="6" style="text-align: center; color: var(--text-muted);">Tidak ada data nilai ditemukan.</td></tr>';
      return;
    }
    ...
    tbody.innerHTML = grades
      .map((n) => {
        ...
        if (["A", "AB", "B"].includes(n.nilai_huruf))
          badgeClass = "badge-status-aktif";
        else if (["BC", "C"].includes(n.nilai_huruf))
          badgeClass = "badge-status-lulus";
        else if (["D"].includes(n.nilai_huruf))
        ...
        else if (n.nilai_huruf === "E" || n.nilai_huruf === "F")
          predikat = "Gagal";
...
function handleGradeCalculation() {
  ...
  if (val >= 85) letterInput.value = "A";
    ...
    letterInput.value = "A-"; // Or matching backend validation (schemas.py allows max_length=2)
...
async function loadFilterDropdowns() {
  ...
  mkFilter.innerHTML =
    '<option value="">-- Filter Mata Kuliah (Semua) --</option>';
...
async function loadNilaiDropdowns(
  ...
  try {
    ...
    students.forEach((m) => {
      ...
      opt.value = m.id;
      opt.textContent = `${m.nim} - ${m.nama}`;
      if (selectedStudentId && parseInt(selectedStudentId) === m.id)
        opt.selected = true;
...
async function saveNilai(e) {
  ...
  const mahasiswa_id = parseInt(
    document.getElementById("nilai-mahasiswa").value,
  );
  const mata_kuliah_id = parseInt(
    document.getElementById("nilai-matakuliah").value,
  );
...
async function editNilai(id) {
  try {
    ...
    document.getElementById("modal-nilai-title").textContent =
...
async function executeDelete() {
  ...
  try {
    ...
    showToast(
      "Berhasil Dihapus",
      res.message || "Data telah dihapus secara permanen.",
      "success",
    );
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  document
    ...
    .addEventListener("submit", saveMahasiswa);
  document.getElementById("form-dosen").addEventListener("submit", saveDosen);
  document
    .getElementById("form-matakuliah")
    .addEventListener("submit", saveMataKuliah);
  document.getElementById("form-nilai").addEventListener("submit", saveNilai);

  // Grade calculation trigger
  document
    .getElementById("nilai-angka")
    .addEventListener("input", handleGradeCalculation);

  // Delete confirm action
  document
    .getElementById("btn-confirm-delete-action")
  ...
  // Add Mata Kuliah
  document
    .getElementById("btn-add-matakuliah")
  ...
  document.querySelectorAll(".quick-action-btn").forEach((btn) => {
    btn.addEventListener("click", () => {
      ...
      if (currentUser && currentUser.role === "mahasiswa") {
        showToast(
          "Akses Ditolak",
          "Mahasiswa tidak diizinkan menambah data.",
          "warning",
        );
        return;
      }
  ...
  document
    .getElementById("btn-reset-filter-nilai")
  ...
  document
    .querySelector('.sidebar-item[data-view="nilai"]')
    .addEventListener("click", loadFilterDropdowns);
});
